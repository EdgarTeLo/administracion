<?php
// CSVProcessor.php - Versión 1.0.17
class CSVProcessor {
    private $conn;
    private $isDebug;
    private $batchSize;
    ...
    public function normalizeDate($dateStr, &$dbData, $field) {
        ...
        // Detectar texto de cancelación o estado
        $cancelationKeywords = ['en proceso de cancelacion', 'cancelada', 'cancleada', 'complemento cancelado', 'factura cancelada'];
        $statusKeywords = ['pagado'];
        $lowerDateStr = strtolower($dateStr);
        if (in_array($lowerDateStr, array_map('strtolower', $cancelationKeywords))) {
            $dbData['estado'] = 'cancelada';
            return null;
        }
        if (in_array($lowerDateStr, $statusKeywords) && $field === 'fecha_pago') {
            $dbData['estado'] = 'pagada';
            return null;
        }
        ...
        // Manejar casos como "15/01/2/0224" o "28/1/2/0205" (interpretar como "15/01/2024" o "28/01/2025")
        ...
        // Manejar casos como "15/01/2/0224" (interpretar como "15/01/2024")
        if (preg_match('/(\d{1,2})\/(\d{1,2})\/(\d{1})\/(\d{4})/', $dateStr, $matches)) {
            $day = $matches[1];
            $month = $matches[2];
            $year = $matches[4];
            $dateStr = "$day/$month/$year";
        ...
        // Corregir formatos como "29/0172024" (interpretar como "29/01/2024")
        if (preg_match('/(\d{1,2})\/(\d{3})(\d{4})/', $dateStr, $matches)) {
            $day = $matches[1];
            $month = substr($matches[2], 0, 2); // Tomar solo los primeros dos dígitos como mes
            $year = $matches[3];
            $dateStr = "$day/$month/$year";
            if ($this->isDebug) {
                debugLog("Fila $globalRowIndex - Fecha $field corregida de " . $matches[0] . " a $dateStr");
            ...
            foreach ($chunks as $chunkIndex => $chunk) {
                ...
                foreach ($chunk as $rowIndex => $row) {
                    ...
                    try {
                        ...
                        // Normalizar fechas con depuración adicional
                        $originalFechaPago = $mappedData['fecha_pago'] ?? 'null';
                        ...
                        if ($this->isDebug) {
                            debugLog("Fila $globalRowIndex - Fecha de pago normalizada: Original='$originalFechaPago', Normalizada='{$dbData['fecha_pago']}'");
                        }

                        // Determinar estado basado en folio_fiscal o descripcion
                        $folioFiscalLower = strtolower($dbData['folio_fiscal'] ?? '');
                        $descripcionLower = strtolower($dbData['descripcion'] ?? '');
                        $cancelationKeywords = ['cancelada', 'cancleada', 'complemento cancelado', 'factura cancelada'];
                        if (in_array($folioFiscalLower, $cancelationKeywords) || in_array($descripcionLower, $cancelationKeywords)) {
                            $dbData['estado'] = 'cancelada';
                        }
                        ...
                        if ($this->isDebug) {
                            debugLog("Fila $globalRowIndex - Parámetros antes de bind_param: " . print_r($params, true));
                        }
                        ...
                        if (!$bindResult) {
                            $errors[] = "Fila $globalRowIndex - Error al vincular parámetros: " . $stmt->error;
                            if ($this->isDebug) debugLog("Error vinculando parámetros: " . $stmt->error);
                            $stmt->close();
                            continue;
                        }
                        ...
                        if (!$stmt->execute()) {
                            ...
                            if ($this->isDebug) debugLog("Error ejecutando SQL: " . $stmt->error . " - Datos: " . print_r($dbData, true));
                        } else {
                            $insertedCount++;
                            if ($this->isDebug) {
                                debugLog("Fila $globalRowIndex: Factura {$dbData['fact']} procesada. Fecha de pago: {$dbData['fecha_pago']}, Contacto: {$dbData['contacto']}, Ubicación: {$dbData['ubicacion']}, Estado: {$dbData['estado']}");
                            }
                        }
                        $stmt->close();
                    } catch (Exception $e) {
